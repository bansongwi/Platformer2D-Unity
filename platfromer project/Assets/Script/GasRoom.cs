...
{
    private bool isGasState = false;            // 자기장의 들어가 있는 상태라면... 플레이어의 채력을 1씩 감소 시킨다.
    ...
    // Tag를 사용해서 Player와 작동 할 수 있도록 작성.
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player") && !isStayOn)
        {
            ...
            Debug.Log($"플레이어의 현재 가스 진입 상태 : {isGasState}");
        }
    }
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.CompareTag("Player") && !isStayOn)
        {
            isGasState = false;                  // Player가 알아야 할 필요성이 있겠죠. PlayerController <- 다른 클래스에서 나의 클래스를 어떻게 접근할 것인가?
            Debug.Log($"플레이어의 현재 가스 진입 상태 : {isGasState}");
        }
    }
    ...
    private void Update()
    { 
        if (isGasState)  // { } 가스 방에 들어깟을때의 로직 작성한다. 체력을 깍이는 로직을 예시로 작성하였다.
        {
